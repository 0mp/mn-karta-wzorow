...
gdzie $\rho_N \coloneqq \flatfrac{\max_{i,j,k}\shortabs{a_{i,j}^{(k)}}}{\max_{i,j} \shortabs{a_{i,j}}}$,
gdzie $a^{(k)}_{i,j} \in A$ w $k$-tym kroku LU. W praktyce wskaźnik wzrostu $\rho_N\leq 2^{N-1}$, więc m. el. G. jest prak. NP.
